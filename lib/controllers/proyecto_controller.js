...
export const create = async (req, res) => {
  try {
    if (
      ...
      //CONTROLAR QUE LOS ESTUDIANTES NO ESTEN ASOCIADOS A UN PROYECTO ACTIVO
      //DEVOLVER EL NOMBRE DEL ALUMNO CUANDO ESTA ASOCIADO
      console.log(req.body.alumnos);
      req.body.alumnos.forEach(async (alu) => {
        console.log(alu.idAlumno);
        const Proyectos = await ListProyectEstudiante(alu.idAlumno);
        console.log(Proyectos.length());
        if (Proyectos.length() > 0) {
          res.status(400).json('El alumno');
        }
      });
...
export const AddEstudentToProject = async (req, res) => {
  try {
    if (req.body.idProyecto !== undefined && req.body.idAlumno !== undefined) {
      ...
      const tokenTemporal = await usuario.generarYGuardarTokenTemporal();
      await sendMail({
